...
const { systemService, backupService, bookingService, notificationService, forecastService } = require("./services");
  ...
  try {
    ...
    setInterval(async () => {
      try {
        ...
        if (now.getDay() !== 6 || now.getHours() !== 23 || now.getMinutes() < 55) return;
        ...
        logger.info(`[ML Export] Weekly export completed: ${result.exported} records`);
      } catch (error) {
